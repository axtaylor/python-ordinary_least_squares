...
def fit(model, adj_cutpoints: bool, y: cp.ndarray, max_iter: int, tol: float):
    ...
    res = newton_raphson_optimize(
        ...
        hessian_gpu,
        start,
        args=(X_scaled, y, model.n_classes),
        max_iter=max_iter,
        tol=tol
    )
...
def negativeLL_gpu(params, X, y, n_classes):
    """
    Negative log-likelihood (GPU optimized).
    """
    params = cp.asarray(params, dtype=cp.float64)
    X = cp.asarray(X, dtype=cp.float64)
    y = cp.asarray(y, dtype=cp.int32)
...
def hessian_gpu(params, X, y, n_classes):
    ...
    Computes the negative expected Hessian of the log-likelihood.
    """
    params = cp.asarray(params, dtype=cp.float64)
    X = cp.asarray(X, dtype=cp.float64)
    y = cp.asarray(y, dtype=cp.int32)
    ...
    # Initialize Hessian
    H = cp.zeros((dim, dim), dtype=cp.float64)
    
    # Compute Hessian using the formula for ordinal logistic regression
    # Based on: I = Σᵢ Σⱼ (dP/dθ)(dP/dθ)ᵀ / P
    
    for i in range(n):
        H_local = cp.zeros((dim, dim), dtype=cp.float64)
        
        # For each category, compute gradient of probability w.r.t. parameters
        for k in range(n_classes):
            pi_k = probs[i, k]
            
            # Gradient of log(P(Y=k|X)) w.r.t. parameters
            grad_k = cp.zeros(dim, dtype=cp.float64)
            
            if k == 0:
                # First category: P(Y=0) = F(alpha_0 - X'beta)
                phi = densities[i, 0]
                grad_k[:p] = phi * X[i]
                grad_k[p] = -phi
            elif k == n_classes - 1:
                # Last category: P(Y=J) = 1 - F(alpha_{J-1} - X'beta)
                phi = densities[i, J - 1]
                grad_k[:p] = -phi * X[i]
                grad_k[p + J - 1] = phi
            else:
                # Middle categories: P(Y=k) = F(alpha_k - X'beta) - F(alpha_{k-1} - X'beta)
                phi_upper = densities[i, k]
                phi_lower = densities[i, k - 1]
                grad_k[:p] = (phi_upper - phi_lower) * X[i]
                grad_k[p + k - 1] = phi_lower
                grad_k[p + k] = -phi_upper
            
            # Fisher information contribution: (grad_k / pi_k) outer product, weighted by pi_k
            # This simplifies to: (grad_k @ grad_k.T) / pi_k
            H_local += cp.outer(grad_k, grad_k) / cp.maximum(pi_k, 1e-10)
